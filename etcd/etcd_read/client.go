...
func main() {
	...
	for i := 0; i < conns; i++ {
		if client, err := clientv3.New(config); err != nil {
			...
			client.Put(context.TODO(), "foo", "bar")
...
func (c *WrkClient) Call() (int64, int64, bool) {
	A := "foo"
	getResp, err := c.Client.Get(context.TODO(), A, clientv3.WithSerializable())
	if err == nil && len(getResp.Kvs) > 0 {
		B := getResp.Kvs[0].Value
		return int64(len(A)), int64(len(B)), true
	}
	return int64(len(A)), 0, false
}
